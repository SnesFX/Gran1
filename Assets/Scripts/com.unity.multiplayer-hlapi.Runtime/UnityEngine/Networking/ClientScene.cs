	...
	public class ClientScene
	{
		...
		private static void OnObjectSpawn(NetworkMessage netMsg)
		{
			...
			else if (NetworkScene.GetPrefab(s_ObjectSpawnMessage.assetId, out prefab))
			{
				...
				{
					uv.Reset();
					ApplySpawnPayload(uv, s_ObjectSpawnMessage.position, s_ObjectSpawnMessage.payload, s_ObjectSpawnMessage.netId, gameObject, netMsg);
				}
			}
			else if (NetworkScene.GetSpawnHandler(s_ObjectSpawnMessage.assetId, out handler))
			{
				...
				else
				{
					...
					ApplySpawnPayload(uv, s_ObjectSpawnMessage.position, s_ObjectSpawnMessage.payload, s_ObjectSpawnMessage.netId, gameObject2, netMsg);
				}
			}
		...
		private static void OnObjectSpawnScene(NetworkMessage netMsg)
		{
			...
			{
				ApplySpawnPayload(uv, s_ObjectSpawnSceneMessage.position, s_ObjectSpawnSceneMessage.payload, s_ObjectSpawnSceneMessage.netId, uv.gameObject, netMsg);
				return;
			}
			...
			ApplySpawnPayload(networkIdentity, s_ObjectSpawnSceneMessage.position, s_ObjectSpawnSceneMessage.payload, s_ObjectSpawnSceneMessage.netId, networkIdentity.gameObject, netMsg);
		}
