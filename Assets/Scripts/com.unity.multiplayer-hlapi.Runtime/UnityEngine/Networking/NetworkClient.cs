	...
	public class NetworkClient
	{
		...
		private void PrepareForConnect(bool usePlatformSpecificProtocols)
		{
			...
			if (m_UseSimulator)
			{
				...
				m_ClientId = NetworkManager.activeTransport.AddHostWithSimulator(m_HostTopology, num, num2, m_HostPort);
			}
			else
			{
				m_ClientId = NetworkManager.activeTransport.AddHost(m_HostTopology, m_HostPort, null);
		...
		internal void ContinueConnect()
		{
			...
			if (m_UseSimulator)
			{
				...
				m_ClientConnectionId = NetworkManager.activeTransport.ConnectWithSimulator(m_ClientId, m_ServerIp, m_ServerPort, 0, out error, conf);
			}
			else
			{
		...
		private void ConnectWithRelay(MatchInfo info)
		{
			m_AsyncConnect = ConnectState.Connecting;
			Update();
			m_ClientConnectionId = NetworkManager.activeTransport.ConnectToNetworkPeer(m_ClientId, info.address, info.port, 0, 0, info.networkId, Utility.GetSourceID(), info.nodeId, out var error);
			m_Connection = (NetworkConnection)Activator.CreateInstance(m_NetworkConnectionClass);
			m_Connection.SetHandlers(m_MessageHandlers);
		...
		internal virtual void Update()
		{
			...
			do
			{
				networkEventType = NetworkManager.activeTransport.ReceiveFromHost(m_ClientId, out var _, out var channelId, m_MsgBuffer, (ushort)m_MsgBuffer.Length, out var receivedSize, out var error);
				if (m_Connection != null)
				{
		...
		public int GetRTT()
		{
			...
			byte error;
			return NetworkManager.activeTransport.GetCurrentRTT(m_ClientId, m_ClientConnectionId, out error);
		}
