...
namespace UnityEngine.Networking
{
	...
	public sealed class NetworkIdentity : MonoBehaviour
	{
		...
		internal void OnStartClient()
		{
			...
			if (LogFilter.logDev)
			{
				Debug.Log(string.Concat("OnStartClient ", base.gameObject, " GUID:", netId, " localPlayerAuthority:", localPlayerAuthority.ToString()));
			}
			for (int i = 0; i < m_NetworkBehaviours.Length; i++)
			{
				NetworkBehaviour networkBehaviour = m_NetworkBehaviours[i];
		...
		public void UNetUpdate()
		{
			...
			for (int i = 0; i < m_NetworkBehaviours.Length; i++)
			{
				int dirtyChannel = m_NetworkBehaviours[i].GetDirtyChannel();
				if (dirtyChannel != -1)
				{
			...
			for (int j = 0; j < NetworkServer.numChannels; j++)
			{
				...
				bool flag = false;
				NetworkBehaviour[] behavioursOfSameChannel = GetBehavioursOfSameChannel(j, initialState: false);
				for (int k = 0; k < behavioursOfSameChannel.Length; k++)
				{
					...
					if (networkBehaviour.OnSerialize(s_UpdateWriter, initialState: false))
					{
					...
					{
						Debug.LogWarning(string.Concat("Large state update of ", s_UpdateWriter.Position - position, " bytes for netId:", netId, " from script:", networkBehaviour));
					}
				}
				if (flag)
				{
		...
		{
			NetworkBehaviour[] behavioursOfSameChannel = GetBehavioursOfSameChannel(netMsg.channelId, initialState);
			for (int i = 0; i < behavioursOfSameChannel.Length; i++)
			{
				behavioursOfSameChannel[i].OnDeserialize(reader, initialState);
			}
		}
		...
		internal void OnNetworkDestroy()
		{
			...
			{
				m_NetworkBehaviours[num].OnNetworkDestroy();
				num++;
			}
			m_IsServer = false;
		}
		...
		internal void ClearObservers()
		{
			if (m_Observers != null)
			{
				...
				{
					m_Observers[i].RemoveFromVisList(this, isDestroyed: true);
				}
				m_Observers.Clear();
		...
		{
			GameObject obj = new GameObject("UNETCallbacks");
			obj.AddComponent(typeof(NetworkCallbacks));
