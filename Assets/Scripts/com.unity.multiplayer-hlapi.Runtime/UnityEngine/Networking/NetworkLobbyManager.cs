...
namespace UnityEngine.Networking
{
	...
	public class NetworkLobbyManager : NetworkManager
	{
		...
		private void OnValidate()
		{
			...
			if (m_LobbyPlayerPrefab != null && m_LobbyPlayerPrefab.GetComponent<NetworkIdentity>() == null)
			{
				m_LobbyPlayerPrefab = null;
				Debug.LogWarning("LobbyPlayer prefab must have a NetworkIdentity component.");
			}
			if (m_GamePlayerPrefab != null && m_GamePlayerPrefab.GetComponent<NetworkIdentity>() == null)
			{
		...
		private static int CheckConnectionIsReadyToBegin(NetworkConnection conn)
		{
			...
			for (int i = 0; i < conn.playerControllers.Count; i++)
			{
				...
				if (playerController.IsValid && playerController.gameObject.GetComponent<NetworkLobbyPlayer>().readyToBegin)
				{
					num++;
				}
			}
		...
		{
			if (SceneManager.GetSceneAt(0).name != m_LobbyScene)
			{
				return;
			}
			int num = 0;
		...
		private void OnGUI()
		{
			if (showLobbyGUI && !(SceneManager.GetSceneAt(0).name != m_LobbyScene))
			{
		...
		public void TryToAddPlayer()
		{
			if (NetworkClient.active)
			{
				...
				if (LogFilter.logDebug)
				{
					Debug.Log("NetworkLobbyManager TryToAddPlayer controllerId " + num + " ready:" + ClientScene.ready.ToString());
				}
				if (num == -1)
				{
					if (LogFilter.logDebug)
