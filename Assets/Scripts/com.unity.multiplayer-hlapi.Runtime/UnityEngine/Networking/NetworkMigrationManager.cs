	...
	public class NetworkMigrationManager : MonoBehaviour
	{
		...
		public void SendPeerInfo()
		{
			...
			for (int i = 0; i < NetworkServer.connections.Count; i++)
			{
				...
				NetworkManager.activeTransport.GetConnectionInfo(NetworkServer.serverHostId, networkConnection.connectionId, out var address, out var port, out var _, out var _, out var _);
		...
		public bool LostHostOnClient(NetworkConnection conn)
		{
			...
			NetworkManager.activeTransport.Disconnect(m_Client.hostId, m_Client.connection.connectionId, out var _);
		...
		protected virtual void OnAuthorityUpdated(GameObject go, int connectionId, bool authorityState)
		{
			if (LogFilter.logDev)
			{
				Debug.Log(string.Concat("NetworkMigrationManager OnAuthorityUpdated for ", go, " conn:", connectionId, " state:", authorityState.ToString()));
			}
		}
