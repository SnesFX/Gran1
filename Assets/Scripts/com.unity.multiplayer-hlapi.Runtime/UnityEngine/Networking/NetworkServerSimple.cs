...
namespace UnityEngine.Networking
{
	...
	public class NetworkServerSimple
	{
		...
		public void ListenRelay(string relayIp, int relayPort, NetworkID netGuid, SourceID sourceId, NodeID nodeId)
		{
			...
			Update();
			NetworkManager.activeTransport.ConnectAsNetworkHost(m_ServerHostId, relayIp, relayPort, netGuid, sourceId, nodeId, out var _);
			m_RelaySlotId = 0;
			if (LogFilter.logDebug)
			{
		...
		public void Update()
		{
			...
			if (m_RelaySlotId != -1)
			{
				networkEventType = NetworkManager.activeTransport.ReceiveRelayEventFromHost(m_ServerHostId, out error);
				if (NetworkEventType.Nothing != networkEventType && LogFilter.logDebug)
				{
			...
			do
			{
				networkEventType = NetworkManager.activeTransport.ReceiveFromHost(m_ServerHostId, out var connectionId, out var channelId, m_MsgBuffer, m_MsgBuffer.Length, out var receivedSize, out error);
		...
		private void HandleConnect(int connectionId, byte error)
		{
			...
			NetworkManager.activeTransport.GetConnectionInfo(m_ServerHostId, connectionId, out var address, out var _, out var _, out var _, out var error2);
