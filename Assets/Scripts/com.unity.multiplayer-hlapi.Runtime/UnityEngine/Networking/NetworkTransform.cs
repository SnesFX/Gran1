...
namespace UnityEngine.Networking
{
	...
	public class NetworkTransform : NetworkBehaviour
	{
		...
		private void UnserializeMode3D(NetworkReader reader, bool initialState)
		{
			...
			if ((m_RigidBody3D.position - m_TargetSyncPosition).magnitude > snapThreshold)
			{
		...
		private void UnserializeMode2D(NetworkReader reader, bool initialState)
		{
			...
			else
			{
				...
				if ((m_RigidBody2D.position - (Vector2)m_TargetSyncPosition).magnitude > snapThreshold)
				{
		...
		private void UnserializeModeCharacterController(NetworkReader reader, bool initialState)
		{
			...
			Vector3 vector = (m_TargetSyncPosition - base.transform.position) / GetNetworkSendInterval();
			m_FixedPosDiff = vector * Time.fixedDeltaTime;
			if (base.isServer && !base.isClient)
			{
				base.transform.position = m_TargetSyncPosition;
			...
			if ((base.transform.position - m_TargetSyncPosition).magnitude > snapThreshold)
			{
				base.transform.position = m_TargetSyncPosition;
			}
			if (interpolateRotation == 0f && syncRotationAxis != 0)
		...
		{
			if (base.syncVarDirtyBits == 0 && NetworkServer.active && base.isServer && GetNetworkSendInterval() != 0f && (!((base.transform.position - m_PrevPosition).magnitude < movementTheshold) || !(Quaternion.Angle(m_PrevRotation, base.transform.rotation) < movementTheshold) || CheckVelocityChanged()))
			{
				SetDirtyBit(1u);
			}
		}
