...
namespace UnityEngine.Networking
{
	...
	public class NetworkTransformChild : NetworkBehaviour
	{
		...
		{
			if (base.syncVarDirtyBits == 0 && NetworkServer.active && base.isServer && GetNetworkSendInterval() != 0f && (!((m_Target.localPosition - m_PrevPosition).sqrMagnitude < movementThreshold) || !(Quaternion.Angle(m_PrevRotation, m_Target.localRotation) < movementThreshold)))
			{
				SetDirtyBit(1u);
			}
		}
		...
		{
			if ((m_Target.localPosition - m_PrevPosition).sqrMagnitude > 1E-05f)
			{
				return true;
			}
			if (Quaternion.Angle(m_Target.localRotation, m_PrevRotation) > 1E-05f)
			{
				return true;
			}
			return false;
